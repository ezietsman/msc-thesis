...
def interpolateHa(im,numvelbins):
    # returns im binned in to numvelbins velocity bins
    # interpolate Ha line to have the same amount of velocity bins as Hb
    # interpolate the image row by row
    imout = []
    for i in range(len(im)):
        vel = N.arange(-750.0,750.0,1500.0/N.shape(im)[1])
        HbVel = N.arange(-750.0,750.0,1500.0/numvelbins)
        cof = sci.splrep(vel,im[i])
        imout.append(sci.splev(HbVel,cof))
...
temp1,temp2 = makebins(imHb,numbins)
Hbsize = N.shape(temp1)[1]
...
print N.shape(im)
#add spectra into velocity bins in order to add balmer lines together.
#im = N.array_split(im,numvelbins,axis=1)
#print type(im)
#im = N.average(im,axis=0)
im = interpolateHa(im,Hbsize)
print N.shape(im)
...
pl.savefig('specgram_fold_ave_sub_lpDNO%s.png' %(round(P2*86400,2)))


pl.figure()
pl.subplots_adjust(wspace=0.001)
ax1 = pl.subplot(121)
pl.imshow((imHa+imHb),aspect='auto',cmap=pl.cm.gray_r,interpolation='nearest',extent=extent)
pl.ylabel('lpDNO Phase')
pl.xlabel('Velocity (km/s)')
pl.title(r'$H_{\alpha}+H_{\beta}$')
pl.xticks(pl.arange(-v,1.2*v,250)[1:-1])

ax3 = pl.subplot(122,sharey=ax1)
pl.imshow(im,aspect='auto',cmap=pl.cm.gray_r,interpolation='nearest',extent=extent)
pl.xlabel('Velocity (km/s)')
pl.title(r'$HeII$')
pl.xticks(pl.arange(-v,1.2*v,250)[1:-1])

yticklabels = ax3.get_yticklabels()# + ax4.get_yticklabels()
pl.setp(yticklabels, visible=False)
pl.savefig('specgram_fold_ave_sub_balmeradded%s.png' %(round(P2*86400,2)))
